...
# Function to preprocess historic price data
...
def generate_initial_prompt(price_data, tweets):
    ...
    prompt += "\nBased on the combined analysis of technical indicators, market trends, sentiment/context, and provided data, predict whether the stock price will go up or down for the next day. Give the response in this format, nothing more:\nPrediction: Rise/Fall."
    return prompt

# Function to generate the daily update prompt with a reminder line
def generate_daily_update_prompt(entry, tweets):
    prompt = """
    Reminder: We are following an incremental learning strategy based on historic price data and recent tweets. While historical data for today's date is not repeated, ensure to maintain and leverage the context of all previous data, including the newly provided historic price data of the previous day. Utilize this cumulative knowledge to predict whether the stock price will rise or fall for the next day. After making your prediction, update the maintained context with the information from today's data. This approach continues with each subsequent prompt.
...
def process_daily_update(date, next_close, entry):
    ...
    daily_prompt = generate_daily_update_prompt(entry, tweets)

    # Send daily update request
    prediction = send_request(daily_prompt)
...
def main():
    ...
    prediction = send_request(initial_prompt)
    ...
    for i, entry in enumerate(price_data[session_data["history_length"]+1:]):  # Start from the 31st day
        ...
        print(next_close)
        process_daily_update(entry['Date'], next_close, entry)
